...
class CallHammerPortal {
  ...
  // ✅ FIXED: Works with ANY n8n Weekly_Payroll output shape
  computeWorkedHoursFromWeeklyPayroll(rangeStart, rangeEnd) {
    ...
    // If n8n returns arrays instead of objects, we can't safely parse here.
    // But most n8n Google Sheets nodes return objects, so we handle that.
    ...
    // ✅ Build date-key candidates (tolerant)
    // We scan keys and treat anything parseable as a date as a date-column
    let total = 0;
    ...
    for (const [k, v] of Object.entries(myRow)) {
      ...
      if (low.includes('agent') || low.includes('employee') || low === 'email' || low.includes('hourly') || low.includes('total') || low.includes('bonus')) {
        // still might be a date column like "Total Hours" etc - but those aren't daily
        // so skip
      }
    ...
    // ✅ Fallback: if backend sends "Total Hours" already computed
    // (some n8n flows may return a Total Hours column only)
...
window.portal = portal;// Call Hammer Leads - Unified Application Logic
class CallHammerPortal {
  constructor() {
    this.currentUser = null;
    this.leadsData = [];
